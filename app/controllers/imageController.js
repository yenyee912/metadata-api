...
exports.uploadImages = async (req, res) => {
  ...
  try {
    if (req.file) {
      ...
      try {
        if (x)
          ...
          res
            .status(304)
            .send({ msg: `Unable to upload image. Please try again.` });
      } catch (err) {
        res
          .status(500)
          .send({ msg: `Error while uploading image: ${err.message}.` });
      }
    } else {
    ...
    res
      .status(500)
      .send({ msg: `Error while uploading image(s): ${err.message}.` });
  }
};
...
exports.getImageByContractAddress = async (req, res, next) => {
  ...
  const x = await Image.findOne({ contractAddress: contractAddress }).sort({
    _id: -1,
  });
  ...
  try {
    ...
    res
      .status(500)
      .send({ msg: `Error while retrieving image(s): ${err.message}.` });
  }
};
...
exports.getOneImage = async (req, res, next) => {
  ...
  try {
    ...
    res.status(500).send({
      msg: `Error while retrieving image(s) with filename= ${fileName}: ${err.message}.`,
    });
  }
};
...
//       res
//         .status(200)
//         .send({ msg: `Successfully delete ${x.n} image(s) record.` });
//     else
//       res
//         .status(304)
//         .send({ msg: `Unable to delete image(s) record. Please try again.` });
//   } catch (err) {
//     res.status(500).send({
//       msg: `Error while deleting image(s) with DO= ${contractAdd}: ${err.message}.`,
//     });
//   }
// };
